...
app.post("/api/bookings", requireUser, (_, res) => res.status(410).json({ error: "Bookings must be initialized through the secure Paystack checkout." }));
app.post("/api/bookings/:id/verify-payment", requireUser, (_, res) => res.status(410).json({ error: "Manual payment verification is disabled. Paystack verifies payments automatically." }));
app.patch("/api/bookings/:id/confirm-payment", requireAdmin, (_, res) => res.status(410).json({ error: "Manual payment confirmation is disabled." }));
app.post("/api/bookings/:id/complete", requireAdmin, async (req, res) => { const { data: booking } = await supabase.from("bookings").select("payment_status").eq("id", req.params.id).single(); if (booking?.payment_status !== "paid") return res.status(409).json({ error: "Paystack must verify the payment before this lesson can be completed." }); const { data, error } = await supabase.from("bookings").update({ status: "completed", payout_status: "ready_for_tutor_payout" }).eq("id", req.params.id).select().single(); return error ? fail(res, error) : res.json(data); });
