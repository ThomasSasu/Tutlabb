...
create table if not exists public.bookings (id uuid primary key default gen_random_uuid(), user_id uuid references public.profiles(id) on delete cascade, tutor_id text references public.tutors(id) on delete set null, details jsonb not null default '{}', price numeric(10,2) not null, people integer not null default 1, currency text not null default 'GHS', amount_subunit bigint, scheduled_at timestamptz, payment_account text, payment_status text not null default 'pending', payment_reference text, payment_proof text, paystack_status text, payment_channel text, gateway_response text, tutor_payout numeric(10,2), platform_share numeric(10,2), status text not null default 'pending_payment', payout_status text, paid_at timestamptz, confirmed_at timestamptz, updated_at timestamptz not null default now(), created_at timestamptz not null default now());
